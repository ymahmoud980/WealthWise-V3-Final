...
export default function ImportDataPage() {
  ...
  const runImport = async () => {
    ...
    try {
      ...
      const existingAssets = currentData.assets || { underDevelopment: [] };
      const existingLiabilities = currentData.liabilities || { installments: [] };
      const existingUnderDev = existingAssets.underDevelopment || [];
      ...
      const processProject = (name: string, developer: string, location: string, base: number, maint: number, park: number, schedule: any[]) => {
        ...
        schedule.forEach((p, i) => {
            // FIX: Default description if missing
            const description = p.desc || "Installment";

            // Full Schedule
            fullSchedule.push({ id: `sch_${i}_${Math.random()}`, date: p.date, amount: p.amount, description });

            if (p.date < today) {
                 // History
                 history.push({ id: `pay_${i}_${Math.random()}`, date: p.date, amount: p.amount, description });
                 paidSum += p.amount;
            } else {
                 // Future
        ...
        if(savedDocs.length > 0) addLog(`Preserving docs for ${name}`);

        // FIX: Safe check for Maintenance Date
        const maintItem = schedule.find((s:any) => s.desc && s.desc.includes("Maintenance"));
        const maintDate = maintItem ? maintItem.date : "";

        return {
            asset: {
                id: oldAsset?.id || uid + "_" + Math.floor(Math.random()*1000), 
                name, location, purchasePrice: base, maintenanceCost: maint, parkingCost: park, currentValue: total, currency: "EGP",
                linkedInstallmentId: lid, maintenanceDueDate: maintDate, paymentFrequency: "Annual",
                documents: savedDocs, 
      ...
      const adminSched = [
        { date: "2022-10-12", amount: 482890, desc: "Downpayment / Initial" }, { date: "2023-07-09", amount: 241500, desc: "Installment" },
        { date: "2024-06-27", amount: 241500, desc: "Installment" }, { date: "2025-07-13", amount: 241500, desc: "Installment" },
        { date: "2025-12-01", amount: 241445, desc: "Maintenance Payment" }, { date: "2026-07-01", amount: 241500, desc: "Upcoming Installment" },
        { date: "2027-07-01", amount: 241500, desc: "Future Installment" }, { date: "2028-07-01", amount: 241500, desc: "Future Installment" },
        { date: "2029-07-01", amount: 241500, desc: "Future Installment" }, { date: "2030-07-01", amount: 241060, desc: "Final Installment" },
      ];
      ...
      const commSched = [
        { date: "2022-10-12", amount: 1689400, desc: "Downpayment / Initial" }, { date: "2023-07-09", amount: 844700, desc: "Installment" },
        { date: "2024-06-27", amount: 844700, desc: "Installment" }, { date: "2025-07-13", amount: 844700, desc: "Installment" },
        { date: "2025-12-21", amount: 928240, desc: "Maintenance Payment" }, { date: "2026-07-01", amount: 844700, desc: "Upcoming Installment" },
        { date: "2027-07-01", amount: 844700, desc: "Future Installment" }, { date: "2028-07-01", amount: 844700, desc: "Future Installment" },
        { date: "2029-07-01", amount: 844700, desc: "Future Installment" }, { date: "2030-07-01", amount: 844687, desc: "Final Installment" },
      ];
      ...
      // 5. TYCOON 2203 (Fixed Descriptions)
      addLog("Processing Tycoon H2203...");
      const t2203Sched = [
        { date: "2023-03-15", amount: 1002205, desc: "Downpayment" }, { date: "2023-09-01", amount: 820000, desc: "Inst 1" },
        { date: "2024-03-01", amount: 820000, desc: "Inst 2" }, { date: "2024-09-01", amount: 820000, desc: "Inst 3" },
        { date: "2025-03-01", amount: 820000, desc: "Inst 4" }, { date: "2025-09-01", amount: 820000, desc: "Inst 5" },
        { date: "2026-03-01", amount: 820000, desc: "Inst 6" }, { date: "2026-09-01", amount: 820000, desc: "Inst 7" },
        { date: "2027-02-01", amount: 1151960, desc: "Maintenance" }, { date: "2027-03-01", amount: 820000, desc: "Inst 8" },
        { date: "2027-09-01", amount: 820000, desc: "Inst 9" }, { date: "2028-03-01", amount: 820000, desc: "Inst 10" },
      ...
      // 6. TYCOON 2222 (Fixed Descriptions)
      addLog("Processing Tycoon H2222...");
      const t2222Sched = [
        { date: "2023-03-15", amount: 948761, desc: "Downpayment" }, { date: "2023-09-01", amount: 776300, desc: "Inst 1" },
        { date: "2024-03-01", amount: 776300, desc: "Inst 2" }, { date: "2024-09-01", amount: 776300, desc: "Inst 3" },
        { date: "2025-03-01", amount: 776300, desc: "Inst 4" }, { date: "2025-09-01", amount: 776300, desc: "Inst 5" },
        { date: "2026-03-01", amount: 776300, desc: "Inst 6" }, { date: "2026-09-01", amount: 776300, desc: "Inst 7" },
        { date: "2027-02-01", amount: 1090530, desc: "Maintenance" }, { date: "2027-03-01", amount: 776300, desc: "Inst 8" },
        { date: "2027-09-01", amount: 776300, desc: "Inst 9" }, { date: "2028-03-01", amount: 776300, desc: "Inst 10" },
      ...
      let cleanAssets = (existingAssets.underDevelopment || []).filter((a: any) => 
         !["Nurai", "Dejoya", "Nile", "Tycoon"].some(k => a.name.includes(k))
      );
      let cleanLiabs = (existingLiabilities.installments || []).filter((l: any) => 
      ...
      setStatus("success");
      addLog("SUCCESS: All Projects Updated.");
  ...
  return (
    <div className="min-h-screen bg-[#020817] text-white p-8 flex flex-col items-center justify-center">
        <h1 className="text-2xl font-bold mb-4">Master Data Importer (Crash Fix)</h1>
        {status === 'idle' && <Button onClick={runImport} className="bg-blue-600 h-12 px-8 font-bold">Update All Projects</Button>}
