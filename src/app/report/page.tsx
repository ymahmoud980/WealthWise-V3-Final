        ...
        <section className="mb-8 break-inside-avoid">
            ...
            <div className="grid grid-cols-2 gap-8">
                <div>
                    ...
                    <div className="flex justify-between text-sm py-1"><span>Salary</span><span>{format(metrics.income.salary)}</span></div>
                    <div className="flex justify-between text-sm py-1"><span>Rental Income</span><span>{format(metrics.income.rent)}</span></div>
                    <div className="flex justify-between font-bold border-t mt-2 pt-1"><span>Total</span><span>{format(metrics.totalIncome)}</span></div>
                </div>
                <div>
                    <h4 className="font-bold border-b mb-2 text-red-700">Outflows</h4>
                    <div className="flex justify-between text-sm py-1"><span>Loan Payments</span><span>{format(metrics.expenses.loans)}</span></div>
                    <div className="flex justify-between text-sm py-1"><span>Household</span><span>{format(metrics.expenses.household)}</span></div>
                    <div className="flex justify-between text-sm py-1"><span>Project Installments (Avg)</span><span>{format(metrics.expenses.installmentsAvg)}</span></div>
                    <div className="flex justify-between font-bold border-t mt-2 pt-1"><span>Total</span><span>{format(metrics.totalExpenses)}</span></div>
                </div>
            </div>
        </section>
