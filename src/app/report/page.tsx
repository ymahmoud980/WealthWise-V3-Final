...
export default function ReportPage() {
  ...
  return (
    <div className="min-h-screen p-8">
      ...
      <div className="mb-8 flex justify-between items-center bg-card/50 backdrop-blur-xl p-4 rounded-xl border border-white/10 text-white">
        <div>
      ...
      {/* --- REPORT CONTENT (White background for clean PDF) --- */}
      <div ref={reportRef} className="max-w-4xl mx-auto bg-white text-black p-10 rounded-sm shadow-2xl">
        ...
        {/* 1. EXECUTIVE SUMMARY (Now includes Cash Flow) */}
            ...
            <table className="w-full text-sm text-left">
                ...
                <tbody>
                    {data.assets.realEstate.map((a, i) => (
                        <tr key={i} className="border-b border-gray-100"><td className="py-1">{a.name} ({a.location})</td><td className="py-1 text-right font-mono">{fmt(a.currentValue, a.currency)}</td></tr>
                    ))}
                    {data.assets.underDevelopment.map((a, i) => (
                        <tr key={`dev-${i}`} className="border-b border-gray-100 text-slate-600"><td className="py-1">{a.name} (Off-Plan)</td><td className="py-1 text-right font-mono">{fmt(a.currentValue, a.currency)}</td></tr>
                    ))}
                    ...
                    {data.assets.gold.map((a, i) => (<tr key={`g-${i}`} className="border-b border-gray-100"><td className="py-1">Gold ({a.grams}g)</td><td className="py-1 text-right font-mono">{fmt(a.grams, 'GOLD_GRAM')}</td></tr>))}
                </tbody>
            </table>
        </section>
