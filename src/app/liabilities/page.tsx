...
export default function LiabilitiesPage() {
  ...
  // ... (Keep handleInstallmentChange, handleLoanChange, handleDeleteConfirm, handleAddLoan, handleAddInstallment logic exactly as is) ...
  const handleInstallmentChange = (id: string, key: string, value: string) => {
      ...
      if(inst) { (inst as any)[key] = ['notes','nextDueDate'].includes(key) ? value : (parseFloat(value)||0); setEditableData(newData); }
  };
  const handleLoanChange = (id: string, key: string, value: string) => {
      const newData = { ...editableData };
      const loan = newData.liabilities.loans.find(l => l.id === id);
      if(loan) { (loan as any)[key] = key === 'notes' ? value : (parseFloat(value)||0); setEditableData(newData); }
  };
    ...
    <div className="space-y-10 pb-20">
      ...
      <Dialog open={!!activeHistory} onOpenChange={() => setActiveHistory(null)}>
        <DialogContent className="bg-[#0f172a] border-white/10 text-white sm:max-w-[500px]">
            <DialogHeader><DialogTitle>Plan: {activeHistory?.project}</DialogTitle></DialogHeader>
            
            <Tabs defaultValue="history" className="w-full">
                <TabsList className="grid w-full grid-cols-2 bg-black/40">
                    <TabsTrigger value="history">Paid History</TabsTrigger>
                    <TabsTrigger value="schedule">Future Schedule</TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="history" className="space-y-4">
                    ...
                    <Button className="w-full bg-emerald-600 hover:bg-emerald-700" onClick={handleAddPayment}><Plus className="h-4 w-4 mr-2"/> Add Manual Payment</Button>
                </TabsContent>

                {/* TAB 2: SCHEDULE */}
                <TabsContent value="schedule" className="space-y-4">
                    <div className="bg-black/40 p-4 rounded-lg border border-white/5 max-h-[400px] overflow-y-auto space-y-2">
                        {activeHistory?.schedule && activeHistory.schedule.length > 0 ? (
                            activeHistory.schedule.map((rec: PaymentRecord, i: number) => {
                                // Check if this specific scheduled item is "Paid" (date in past)
                                const isPast = new Date(rec.date) < new Date();
                                return (
                                    <div key={i} className={`flex justify-between items-center p-2 rounded border ${isPast ? 'bg-emerald-900/10 border-emerald-500/10 opacity-50' : 'bg-white/5 border-white/10'}`}>
                                        <div className="flex gap-3 items-center">
                                            <div className="w-20 text-[10px] text-muted-foreground">{rec.date}</div>
                                            <div>
                                                <p className={`text-sm font-bold ${isPast ? 'text-emerald-500' : 'text-white'}`}>{formatNumber(rec.amount)}</p>
                                                <p className="text-[10px] text-muted-foreground">{rec.description}</p>
                                            </div>
                                        </div>
                                        {isPast && <span className="text-[10px] bg-emerald-500/20 text-emerald-500 px-2 py-0.5 rounded">PAID</span>}
                                    </div>
                                )
                            })
                        ) : <p className="text-center text-sm text-muted-foreground">No schedule imported.</p>}
                    </div>
                </TabsContent>
            </Tabs>
