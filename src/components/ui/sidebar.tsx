  ...
  return (
    <div className="flex flex-col h-full bg-[#111827] text-white relative w-full">
      ...
      <div className="p-4 border-b border-white/10 shrink-0 flex items-center justify-between bg-[#111827] z-10">
        ...
        {onClose && <Button variant="ghost" size="icon" className="md:hidden text-zinc-400" onClick={onClose}><X className="h-5 w-5" /></Button>}
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar px-3 py-4">
        <div className="space-y-1">
          {routes.map((route) => (
            <Link key={route.href} href={route.href} onClick={onClose} className={cn("text-sm group flex p-3 w-full justify-start font-medium cursor-pointer hover:text-white hover:bg-white/10 rounded-lg transition", pathname === route.href ? "text-white bg-white/10" : "text-zinc-400")}>
              <div className="flex items-center flex-1"><route.icon className={cn("h-5 w-5 mr-3 shrink-0", route.color)} /><span className="truncate">{route.label}</span></div>
            </Link>
          ))}
        </div>
      </div>
