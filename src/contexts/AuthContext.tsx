...
import { 
  ...
  onAuthStateChanged, 
  setPersistence, 
  browserSessionPersistence, // <--- The Secret Ingredient
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // 1. Enforce Session Persistence (Logout on Close)
  ...
  const loginWithGoogle = async () => {
    try {
      ...
      if (error.code === 'auth/popup-closed-by-user') return; // Ignore popup close
      ...
      <div className="flex min-h-screen items-center justify-center bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-[#0f172a] to-black p-4">
        <Card className="w-full max-w-md bg-white/5 border-white/10 backdrop-blur-md">
          ...
          <CardContent>
            ...
            <p className="text-xs text-center text-muted-foreground mt-4">Session expires when window closes.</p>
          </CardContent>
        </Card>
      </div>
    );
