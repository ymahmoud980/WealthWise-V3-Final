...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // 1. Listen for Real Authentication Status
  useEffect(() => {
  ...
  // 2. Google Login (Improved Error Handling)
  const loginWithGoogle = async () => {
    try {
      await signInWithPopup(auth, googleProvider);
    } catch (error: any) {
      // FIX: Ignore the error if the user simply closed the window
        ...
        <Card className="w-full max-w-md bg-white/5 border-white/10 backdrop-blur-md">
          ...
          <CardContent>
            <Button 
              ...
              // Force white button for visibility
            ...
            <p className="text-xs text-center text-muted-foreground mt-4">
              Data is encrypted and synced across your devices.
            </p>
          </CardContent>
        </Card>
      </div>
